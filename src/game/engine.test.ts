...
import { buyDesertSector, claimContract, createInitialState, demolitionRefund, globalSalesCapacity, globalStorageCapacity, isComponentUnlocked, placeTile, refuelTile, repairTile, sectorEnergyStored, sellStoredEnergy, sellTile, isAutoRebuildActive, simulateMany, simulateSecond, simulateTick, switchSector, totalHeat } from './engine'
...
describe('economy v2', () => {
  it('starts with the promoted credits and separate resource stores', () => { const state = createInitialState(); expect(state.credits).toBe(ECONOMY.startingCredits); expect(state.credits).toBe(1); expect(sectorEnergyStored(state)).toBe(0); expect(state.researchPoints).toBe(0); expect(state.version).toBe(21) })
  ...
  it('actively pumps heat through a continuous Pipe II route toward converter demand', () => { let state = unlocked(); state = placeTile(state, 0, 'core'); state = placeTile(state, 1, 'pipe2'); state = placeTile(state, 2, 'pipe2'); state = placeTile(state, 3, 'generator'); const result = simulateTick(state); expect(result.report.thermalEnergy).toBeCloseTo(conversionRate(state)); expect(result.state.tiles[1]?.flow).toBeGreaterThan(0); expect(result.state.tiles[2]?.flow).toBeGreaterThan(0); expect(totalHeat(result.state) + result.report.thermalEnergy).toBeCloseTo(COMPONENTS.core.production!, 6) })
  ...
  it('never rescales existing infrastructure when Thorium or Fusion unlocks', () => {
    ...
    for (const kind of ['pipe', 'pipe2', 'exchanger', 'accumulator', 'generator', 'generator2'] as const) {
  ...
  it('refunds nothing for durable producers and 85 percent for other pieces', () => { for (const kind of LIFETIME_ORDER) expect(demolitionRefund(kind)).toBe(0); expect(demolitionRefund('battery')).toBe(42_500); let state = unlocked(); state = placeTile(state, 0, 'wind'); const afterBuild = state.credits; state = sellTile(state, 0); expect(state.credits).toBe(afterBuild); state = placeTile(state, 0, 'battery'); const beforeSale = state.credits; state = sellTile(state, 0); expect(state.credits - beforeSale).toBe(42_500) })
  ...
  it('uses the promoted auto rebuild research prices', () => { expect(AUTO_REBUILD_COSTS).toEqual({ wind: 15, solar: 3_000, core: 150_000, thorium: 9_000_000, fusion: 60_000_000 }) })
  ...
  it('upgrades every unit of a type on the active map while build cost stays fixed', () => { let state = rich(); const cost = upgradeCost(state, 'wind'); state = upgradeBuildingType(state, 'wind'); expect(state.sectorEconomies.coast.buildingLevels.wind).toBe(2); expect(state.sectorEconomies.desert.buildingLevels.wind).toBe(0); expect(rich().credits - state.credits).toBe(cost); expect(COMPONENTS.wind.cost).toBe(1) })
  it('starts Distrito Neón at level zero, keeps Costa upgrades, and reaches level fifteen', () => {
    let state = rich()
    state = upgradeBuildingTrack(state, 'wind', 'output')
    state = upgradeBuildingTrack(state, 'wind', 'output')
    expect(componentLevel(state, 'wind')).toBe(3)
    state = switchSector({ ...state, ownedSectors: { coast: true, desert: true } }, 'desert')
    expect(componentLevel(state, 'wind')).toBe(0)
    expect(directEnergyRate(state, 'wind')).toBeCloseTo(COMPONENTS.wind.directEnergy!)
    expect(maxUpgradeLevel(state)).toBe(15)
    for (let level = 0; level < 15; level += 1) state = upgradeBuildingTrack(state, 'wind', 'output')
    expect(componentLevel(state, 'wind')).toBe(15)
    expect(directEnergyRate(state, 'wind')).toBeCloseTo(COMPONENTS.wind.directEnergy! * levelMultiplier(16))
    expect(upgradeBuildingTrack(state, 'wind', 'output')).toBe(state)
    state = switchSector(state, 'coast')
    expect(componentLevel(state, 'wind')).toBe(3)
    expect(maxUpgradeLevel(state)).toBe(10)
  })
  ...
  it('turns a Pipe II output upgrade into more active throughput and lower positive resistance', () => { const state = rich(); const beforeTransfer = thermalTransferRate(state, 'pipe2'); const beforeResistance = thermalResistance(state, 'pipe2'); const upgraded = upgradeBuildingTrack(state, 'pipe2', 'output'); expect(thermalTransferRate(upgraded, 'pipe2') / beforeTransfer).toBeCloseTo(1.35); expect(beforeResistance / thermalResistance(upgraded, 'pipe2')).toBeCloseTo(1.35); expect(thermalResistance(upgraded, 'pipe2')).toBeGreaterThan(0) })
...
describe('storage safety', () => {
  ...
  it('keeps the old save key untouched while evolving the v2 model', () => { expect(createInitialState().version).toBe(21) })
})
