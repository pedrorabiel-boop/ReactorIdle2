...
describe('economy v2', () => {
  it('starts with five credits and separate resource stores', () => { const state = createInitialState(); expect(state.credits).toBe(5); expect(sectorEnergyStored(state)).toBe(0); expect(state.researchPoints).toBe(0); expect(state.version).toBe(14) })
