...
export function DebugSheet({ game, onClose, onChange, onSetCredits, onSetResearch, onReset }: Props) {
  ...
  return <Sheet title="Debug" eyebrow="SANDBOX DE BALANCE" onClose={onClose} className="debug-sheet">
    ...
    <section className="debug-section"><h3>Recursos y sandbox</h3><button className={`btn frame wide ${settings.infiniteMoney ? 'on gold' : ''}`} onClick={() => onChange({ ...settings, infiniteMoney: !settings.infiniteMoney })}>Dinero infinito: {settings.infiniteMoney ? 'ACTIVO' : 'NO'}</button><button className={`btn frame wide ${settings.unlockAllBuildings ? 'on gold' : ''}`} onClick={() => onChange({ ...settings, unlockAllBuildings: !settings.unlockAllBuildings })}>Sandbox · todas las torres: {settings.unlockAllBuildings ? 'ACTIVO' : 'NO'}</button><p className="hint">Desbloquea temporalmente todos los edificios sin comprar tecnologías. Al apagarlo vuelven los bloqueos normales.</p><div className="debug-grid"><NumberField label="Dinero actual" value={game.credits} onChange={onSetCredits} /><NumberField label="RP actuales" value={game.researchPoints} onChange={onSetResearch} /><NumberField label="Dinero inicial al reiniciar" value={settings.initialCredits} onChange={(initialCredits) => onChange({ ...settings, initialCredits })} /></div></section>
