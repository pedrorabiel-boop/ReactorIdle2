...
function App() {
  ...
  function startBuildStroke(event: ReactPointerEvent<HTMLButtonElement>, index: number) { const current = gameRef.current; if (!armedRef.current || current.toolMode !== 'build' || event.button !== 0) return; ignoreClickRef.current = true; event.preventDefault(); strokeRef.current = { pointerId: event.pointerId, visited: new Set(), beforeTiles: current.tiles, spent: 0, placed: 0 }; placeDuringStroke(index) }
  ...
  return <div className={`app env-${environment}`} style={environmentStyle(environment)}><SpriteDefs />
    ...
    {!buildFocus && <Dock game={game} activeTab={activeTab} onTab={openTab} onChooseComponent={chooseComponent} onChooseTool={chooseTool} onUndo={undo} undoDepth={undoDepth} labBadge={affordableTechs} upgradesBadge={affordableUpgrades} contractReady={contractComplete} />}
